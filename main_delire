...
//Cree la forme ...
    ...
    switch(type) //... en fonction du type
    {
        ...
        case TYPE_SHAPE_CYLINDRE: //Un cylindre
         ...
         case TYPE_SHAPE_BITE: //Une c̫ne
            pShape = malloc(sizeof(TShapeBite));
            if(pShape==NULL)
            {
                printf("Erreur pour creer la forme Bite\n");
                return;
            }
            printf("Rayon: ");
            scanf("%f",&r0);
            ((TShapeBite*)pShape)->m_Radius = r0;
            printf("Hauteur: ");
            scanf("%f",&h0);
            ((TShapeBite*)pShape)->m_Height = h0;
            printf("Nombre de secteur: ");
            scanf("%d",&i0);
            ((TShapeBite*)pShape)->m_Sector = i0;
            pF_Draw = DrawBite;
            break;
